...
class TestReportedMaps(TestSettings):
    ...
    def test_rep_dischargemaps(self, mocker):
        self._reported_map(self.settings_file, ['repDischargeMaps'], None, 'DischargeMaps', mocker)

    def test_rep_dischargemaps_not_called(self, mocker):
        self._not_reported_map(self.settings_file, ['repEndMaps'], ['simulateLakes'], 'DischargeMaps', mocker)

    def test_rep_lakes(self, mocker):
        self._reported_map(self.settings_file_lakes, ['simulateLakes', 'repsimulateLakes', 'repEndMaps', 'repStateMaps'],
                           None, 'LakeLevelState', mocker)

    def test_rep_lakes_not_called(self, mocker):
        self._not_reported_map(self.settings_file_lakes, ['repEndMaps', 'repStateMaps'], None, 'LakeLevelState', mocker)

    def test_rep_reservoirs(self, mocker):
        self._reported_map(self.settings_file, ['simulateReservoirs', 'repsimulateReservoirs', 'repStateMaps'],
                           ['simulateLakes', 'repsimulateLakes'], 'ReservoirFillState', mocker)

    def test_rep_reservoirs_not_called(self, mocker):
        self._not_reported_map(self.settings_file, opts_to_set=['repStateMaps'],
    ...
    def test_rep_pfmaps(self, mocker):
        self._reported_map(self.settings_file,
    ...
    def test_rep_lzmaps(self, mocker):
        self._reported_map(self.settings_file, opts_to_set=['repLZMaps', 'repE2O2'],
                           opts_to_unset=['simulateLakes', 'repsimulateLakes'],
                           map_to_check='LZMaps', mocker=mocker)

    def test_rep_lzmaps_not_called(self, mocker):
        self._not_reported_map(self.settings_file, opts_to_set=['repEndMaps', ],
    ...
    def test_rep_gwpercuzlzmaps_not_called(self, mocker):
        self._not_reported_map(self.settings_file, opts_to_set=['repEndMaps', 'repStateMaps',],
                           opts_to_unset=['simulateLakes', 'repsimulateLakes'],
    ...
    def test_rep_windex_not_called(self, mocker):
        self._not_reported_map(self.settings_file, opts_to_set=['repStateMaps'], opts_to_unset=['simulateLakes', 'repsimulateLakes'],
