...
#        import ipdb; ipdb.set_trace()
    ...
    def dynamic(self, sLoop):
        ...
        if option['simulatePF']:
            ...
            ## EDITED BELOW: residual water content and saturated water content for layers 1a and 1b (and NOT for layer 1 = a+b)
            
            SatTerm1a = np.where(self.var.PoreSpaceNotZero1a[sLoop], (self.var.W1a[
                                                                          sLoop] - self.var.WRes1a[sLoop]) / (
                                             self.var.WS1a[sLoop] - self.var.WRes1a[sLoop]), maskinfo.in_zero())
            SatTerm1b = np.where(self.var.PoreSpaceNotZero1b[sLoop], (self.var.W1b[
                                                                          sLoop] - self.var.WRes1b[sLoop]) / (
                                             self.var.WS1b[sLoop] - self.var.WRes1b[sLoop]), maskinfo.in_zero())   
