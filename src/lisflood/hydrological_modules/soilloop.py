...
from ..global_modules.settings import VEGETATION_LANDUSE, LANDUSE_VEGETATION, PRESCRIBED_VEGETATION, PRESCRIBED_LAI, SOIL_USES
...
#        import ipdb; ipdb.set_trace()
        ...
        self.index_landuse_all = np.array([SOIL_USES.index(VEGETATION_LANDUSE[v]) for v in self.var.vegetation])
        self.index_landuse_prescr = np.array([SOIL_USES.index(VEGETATION_LANDUSE[v]) for v in self.var.prescribed_vegetation])
        self.is_irrigated = np.array([VEGETATION_LANDUSE[v] == 'Irrigated' for v in self.var.vegetation])
        if option["cropsEPIC"]:
    ...
    def dynamic_canopy(self):
        ...
        for veg in self.var.prescribed_vegetation:
            landuse = VEGETATION_LANDUSE[veg] # landuse corresponding to vegetation fraction
