...
def soilColumnsWaterBalance(index_landuse_all, is_irrigated, is_paddy_irrig, paddy_inactive, DtDay,
    ...
    for veg in range(num_vegs):
        ...
        for q in prange(sim_pixels.size):
            ...
            RelSat1 = min(W1[veg,pix] / WS1[landuse,pix], 1.) if PoreSpaceNotZero1a[landuse,pix] else 0.
            ...
            SatFraction = 1. - (1. - RelSat1) ** b_Xinanjiang[pix]
            ...
            InfiltrationPot = 0. if isFrozenSoil[pix] else StoreMaxPervious[landuse,pix] * (1. - SatFraction) ** PowerInfPot[pix]
...
#        import ipdb; ipdb.set_trace()
    ...
    def dynamic(self, sLoop):
        """dynamic method (no epic activated)"""
        # ************************************************************
        # ***** INTERCEPTION *****************************************
        # ************************************************************
        ...
        SMax = np.where(self.var.LAI[sLoop] > 0.1,
                        0.935 + 0.498 * self.var.LAI[sLoop] - 0.00575 * np.square(self.var.LAI[sLoop]),
                        maskinfo.in_zero())
        ...
        self.var.Interception[sLoop] = np.where(SMax > 0.0,
                                                np.minimum(SMax - self.var.CumInterception[sLoop],
                                                           SMax * (1 - np.exp(
                                                               -0.046 * self.var.LAI[sLoop] * self.var.Rain / SMax))),
                                                maskinfo.in_zero())
        ...
        self.var.CumInterception[sLoop] = np.maximum(self.var.CumInterception[sLoop] - self.var.TaInterception[sLoop],
                                                     maskinfo.in_zero())
        # evaporated water is subtracted from Cumulative Interception
        ...
        # assumption: after 1 day all intercepted water is evaporated or has fallen
        # on the soil surface
        self.var.CumInterception[sLoop] = np.maximum(self.var.CumInterception[sLoop] - self.var.LeafDrainage[sLoop],
                                                     maskinfo.in_zero())
        ...
        self.var.AvailableWaterForInfiltration[sLoop] = \
            np.maximum(self.var.Rain + self.var.SnowMelt + self.var.LeafDrainage[sLoop] - self.var.Interception[sLoop],
                       maskinfo.in_zero())
        # Water available for infiltration during this timestep [mm]
        ...
        # CropGroupNumber 1-5
        p = np.where(self.var.CropGroupNumber[sLoop] <= 2.5,
                     p + (np.minimum(0.1 * self.var.ETRef * self.var.InvDtDay, 1.0) - 0.6) / (
        ...
        self.var.RWS[sLoop] = np.where((WCrit1 - self.var.WWP1[sLoop]) > 0,
                                       (self.var.W1[sLoop] - self.var.WWP1[sLoop]) / (WCrit1 - self.var.WWP1[sLoop]),
                                       1.0)
        ...
        if option['repStressDays']:
            self.var.SoilMoistureStressDays[sLoop] = np.where(self.var.RWS[sLoop] < 1, self.var.DtDay,
                                                              maskinfo.in_zero())
        ...
        self.var.Ta[sLoop] = np.maximum(
            np.minimum(self.var.RWS[sLoop] * self.var.TranspirMaxCorrected, self.var.W1[sLoop] - self.var.WWP1[sLoop]),
            0.0)
        # actual transpiration based on both layers 1a and 1b
        self.var.Ta[sLoop] = np.where(self.var.FrostIndex > self.var.FrostIndexThreshold, maskinfo.in_zero(),
                                      self.var.Ta[sLoop])
        ...
        wc1a = np.maximum(self.var.W1a[sLoop] - WCrit1a,
                          0)  # unstressed water availability from layer 1a without stress (above critical soil moisture)
        wc1b = np.maximum(self.var.W1b[sLoop] - WCrit1b, 0)  # (same as above but for layer 1b)
        Ta1a = np.minimum(self.var.Ta[sLoop],
                          wc1a)  # temporary transpiration from layer 1a (<= unstressed layer 1a availability)
        restTa = np.maximum(self.var.Ta[sLoop] - Ta1a,
                            0)  # transpiration left after layer 1a unstressed water has been abstracted
        Ta1b = np.minimum(restTa, wc1b)  # temporary transpiration from layer 1b (<= unstressed layer 1b availability)
        restTa = np.maximum(restTa - Ta1b,
                            0)  # transpiration left after layers 1a and 1b unstressed water have been abstracted
        ...
        fraction_rest_1a = np.where(available, stressed_availability_1a / stressed_availability_tot,
                                    0)  # |> proportionally to each root-zone layer (1a and 1b)
        fraction_rest_1b = np.where(available, stressed_availability_1b / stressed_availability_tot,
        ...
        self.var.DSLR[sLoop] = np.where(self.var.AvailableWaterForInfiltration[
                                            sLoop] > self.var.AvWaterThreshold, 1.0,
        ...
        # either ESAct or availabe water from layer 1a and 1b
        self.var.ESAct[sLoop] = np.where(self.var.FrostIndex > self.var.FrostIndexThreshold, maskinfo.in_zero(),
        ...
        InfiltrationPot = np.where(self.var.FrostIndex > self.var.FrostIndexThreshold, maskinfo.in_zero(),
                                   InfiltrationPot)
        # When the soil is frozen (frostindex larger than threshold), potential
        # infiltration is zero
        ...
        self.var.Infiltration[sLoop] = np.maximum(
            np.minimum(self.var.AvailableWaterForInfiltration[sLoop], InfiltrationPot), maskinfo.in_zero())
        # infiltration in [mm] per timestep
        # Maximum infiltration is equal to Rainfall-Interception-Snow+Snowmelt
        ...
        self.var.W1b[sLoop] = self.var.W1b[sLoop] + np.where(testW1a > self.var.WS1a[sLoop],
                                                             testW1a - self.var.WS1a[sLoop], maskinfo.in_zero())
        ...
        KUnSat1a, KUnSat1b, KUnSat2 = self.unsaturatedConductivity(
            sLoop)  # Unsaturated conductivity at the beginning of this time step [mm/day]
        ...
        CourantTopToSubA = np.where(AvailableWater1a == 0, maskinfo.in_zero(),
                                    KUnSat1a * self.var.DtDay / AvailableWater1a)
        CourantTopToSubB = np.where(AvailableWater1b == 0, maskinfo.in_zero(),
        ...
        for i in range(self.var.NoSubSteps):
            if i > 0:
                KUnSat1a, KUnSat1b, KUnSat2 = self.unsaturatedConductivity(sLoop, (
                WTemp1a, WTemp1b, WTemp2))  # Unsaturated conductivity [mm/day]
        ...
        self.var.SeepTopToSubA[sLoop] = np.where(self.var.FrostIndex > self.var.FrostIndexThreshold, maskinfo.in_zero(),
                                                 self.var.SeepTopToSubA[sLoop])
        self.var.SeepTopToSubB[sLoop] = np.where(self.var.FrostIndex > self.var.FrostIndexThreshold, maskinfo.in_zero(),
                                                 self.var.SeepTopToSubB[sLoop])
        # When the soil is frozen (frostindex larger than threshold), seepage
        # is zero
        self.var.SeepSubToGW[sLoop] = np.where(self.var.FrostIndex > self.var.FrostIndexThreshold, maskinfo.in_zero(),
                                               self.var.SeepSubToGW[sLoop])
        # When the soil is frozen (frostindex larger than threshold), seepage
        # is zero
        ...
        self.var.Infiltration[sLoop] = self.var.Infiltration[sLoop] - np.maximum(
        ...
        self.var.Sat1a[sLoop] = (self.var.W1a[sLoop] - self.var.WWP1a[sLoop]) / (
                    self.var.WFC1a[sLoop] - self.var.WWP1a[sLoop])
        self.var.Sat1b[sLoop] = (self.var.W1b[sLoop] - self.var.WWP1b[sLoop]) / (
                    self.var.WFC1b[sLoop] - self.var.WWP1b[sLoop])
        ...
        if option['simulatePF']:
            SatTerm1a = np.where(self.var.PoreSpaceNotZero1a[sLoop], (self.var.W1a[
                                                                          sLoop] - self.var.WRes1[sLoop]) / (
                                             self.var.WS1[sLoop] - self.var.WRes1[sLoop]), maskinfo.in_zero())
            ...
            SatTerm2 = np.where(self.var.PoreSpaceNotZero2[sLoop], (self.var.W2[
                                                                        sLoop] - self.var.WRes2[sLoop]) / (
                                            self.var.WS2[sLoop] - self.var.WRes2[sLoop]), maskinfo.in_zero())
        ...
        if option['drainedIrrigation'] and sLoop == 2:
            ...
            self.var.UZ[sLoop] += (1 - self.var.DrainedFraction) * self.var.SeepSubToGW[sLoop] + self.var.PrefFlow[
                sLoop]
    ...
    def dynamic_canopy(self):
        ...
        if self.settings.option.get('cropsEPIC'):
        ...
        for veg in self.var.prescribed_vegetation:
            ...
            swdf = 1 / (0.76 + 1.5 * np.minimum(0.1 * self.var.ETRef * self.var.InvDtDay, 1.0)) - 0.10 * (5 - self.var.CropGroupNumber.loc[landuse])
            # soil water depletion fraction (easily available soil water)
            # Van Diepen et al., 1988: WOFOST 6.0, p.87
            # to avoid a strange behaviour of the p-formula's, ETRef is set to a maximum of
            # 10 mm/day. Thus, p will range from 0.15 to 0.45 at ETRef eq 10 and
            # CropGroupNumber 1-5
            swdf = np.where(self.var.CropGroupNumber.loc[landuse] <= 2.5, swdf + (np.minimum(0.1 * self.var.ETRef * self.var.InvDtDay, 1.0) - 0.6) / (
                self.var.CropGroupNumber.loc[landuse] * (self.var.CropGroupNumber.loc[landuse] + 3)), swdf)
            # correction for crop groups 1 and 2 (Van Diepen et al, 1988)
            swdf = np.maximum(np.minimum(swdf, 1.0), 0)
            # p is between 0 and 1
            WCrit1 = ((1 - swdf) * (self.var.WFC1.loc[landuse] - self.var.WWP1.loc[landuse])) + self.var.WWP1.loc[landuse]
            WCrit1a = ((1 - swdf) * (self.var.WFC1a.loc[landuse] - self.var.WWP1a.loc[landuse])) + self.var.WWP1a.loc[landuse]
            WCrit1b= ((1 - swdf) * (self.var.WFC1b.loc[landuse] - self.var.WWP1b.loc[landuse])) + self.var.WWP1b.loc[landuse]
            # critical moisture amount ([mm] water slice) for all layers
            if self.settings.option['wateruse']:
            ...
            self.var.RWS.loc[veg] = np.where((WCrit1 - self.var.WWP1.loc[landuse]) > 0,\
                                             (self.var.W1.loc[veg] - self.var.WWP1.loc[landuse]) / (WCrit1 - self.var.WWP1.loc[landuse]), 1)
            # Transpiration reduction factor (in case of water stress)
            # if WCrit1 = WWP1, RWS is zero there is no water stress in that case
            self.var.RWS.loc[veg] = np.maximum(np.minimum(self.var.RWS.loc[veg], 1), 0)
            # Transpiration reduction factor (in case of water stress)
            if self.settings.option['repStressDays']:
                self.var.SoilMoistureStressDays.loc[veg] = np.where(self.var.RWS.loc[veg] < 1, self.var.DtDay, 0)
                # Count number of days with soil water stress, RWS is between 0 and 1
                # no reduction of Transpiration at RWS=1, at RWS=0 there is no Transpiration at all

            transpirable_water = np.maximum(self.var.W1.loc[veg] - self.var.WWP1.loc[landuse], 0)
            self.var.Ta.loc[veg] = np.minimum(self.var.RWS.loc[veg] * self.var.potential_transpiration.loc[veg], transpirable_water)
            # actual transpiration based on both layers 1a and 1b
            self.var.Ta.loc[veg] = np.where(self.var.isFrozenSoil, 0, self.var.Ta.loc[veg])
            # transpiration is 0 when soil is frozen
            # calculate distribution where to take Ta from:
            # 1st: above wCrit from layer 1a
            # 2nd: above Wcrit from layer 1b
            # 3rd:  distribute take off according to soil moisture availability below wcrit
            wc1a = np.maximum(self.var.W1a.loc[veg] - WCrit1a, 0) # unstressed water availability from layer 1a without stress (above critical soil moisture)
            wc1b = np.maximum(self.var.W1b.loc[veg] - WCrit1b, 0) # (same as above but for layer 1b)
            Ta1a = np.minimum(self.var.Ta.loc[veg], wc1a)         # temporary transpiration from layer 1a (<= unstressed layer 1a availability)
            restTa = np.maximum(self.var.Ta.loc[veg] - Ta1a, 0)   # transpiration left after layer 1a unstressed water has been abstracted
            Ta1b = np.minimum(restTa, wc1b)                       # temporary transpiration from layer 1b (<= unstressed layer 1b availability)
            restTa = np.maximum(restTa - Ta1b, 0)                 # transpiration left after layers 1a and 1b unstressed water have been abstracted
            stressed_availability_1a = np.maximum(self.var.W1a.loc[veg] - Ta1a - self.var.WWP1a.loc[landuse], 0) #|
            stressed_availability_1b = np.maximum(self.var.W1b.loc[veg] - Ta1b - self.var.WWP1b.loc[landuse], 0) #|
            stressed_availability_tot = stressed_availability_1a + stressed_availability_1b                      #|> distribution of abstractions of
            available = stressed_availability_tot > 0                                                            #|> soil moisture below the critical value
            ...
            Ta1a += fraction_rest_1a * restTa                                                                    #|
            Ta1b += fraction_rest_1b * restTa                                                                    #|
            self.var.W1a.loc[veg] -= Ta1a
            self.var.W1b.loc[veg] -= Ta1b
            self.var.W1.loc[veg] = self.var.W1a.loc[veg] + self.var.W1b.loc[veg]
    ...
    def dynamic_soil(self):
        ...
        if self.settings.option['simulatePF']:
    ...
    def ThetaSatTerms(self, veg):
        ...
        self.var.Theta1a.loc[veg] = thetaFunVectorized(self.var.W1a.loc[veg].values, self.var.SoilDepth1a.loc[landuse].values,
                                                       self.var.PoreSpaceNotZero1a.loc[landuse].values)
        self.var.Theta1b.loc[veg] = thetaFunVectorized(self.var.W1b.loc[veg].values, self.var.SoilDepth1b.loc[landuse].values,
                                                       self.var.PoreSpaceNotZero1b.loc[landuse].values)
        self.var.Theta2.loc[veg] = thetaFunVectorized(self.var.W2.loc[veg].values, self.var.SoilDepth2.loc[landuse].values,
                                                      self.var.PoreSpaceNotZero2.loc[landuse].values)
        self.var.Sat1a.loc[veg] = satFunVectorized(self.var.W1a.loc[veg].values, self.var.WWP1a.loc[landuse].values,
                                                   self.var.WFC1a.loc[landuse].values)
        self.var.Sat1b.loc[veg] = satFunVectorized(self.var.W1b.loc[veg].values, self.var.WWP1b.loc[landuse].values,
        ...
        self.var.Sat2.loc[veg] = satFunVectorized(self.var.W2.loc[veg].values, self.var.WWP2.loc[landuse].values,
