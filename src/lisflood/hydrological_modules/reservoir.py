...
class reservoir(HydroModule):
    ...
    def dynamic_inloop(self, NoRoutingExecuted: int):
        ...
        if option['simulateReservoirs'] and not option['InitLisflood']:
            ...
            # reservoir characteristics
            min_outflow, norm_outflow, flood_outflow = self.var.minReservoirOutflowCC, self.var.NormalReservoirOutflowCC, self.var.NonDamagingReservoirOutflowCC
            cons_fill, norm_fill, adj_norm_fill, flood_fill = self.var.ConservativeStorageLimitCC, self.var.NormalStorageLimitCC, self.var.Normal_FloodStorageLimitCC, self.var.FloodStorageLimitCC
            ...
            outflow = np.where(
                ...
                min_outflow + (norm_outflow - min_outflow) * (self.var.ReservoirFillCC - 2 * cons_fill) / (norm_fill - 2 * cons_fill),
                outflow
            )
            # normal zone
            outflow = np.where(
                self.var.ReservoirFillCC > norm_fill,
                norm_outflow,
                outflow
            )
            # NEW 24-9-2004: linear transition between normal and non-damaging outflow
            outflow = np.where(
                self.var.ReservoirFillCC > adj_norm_fill,
                norm_outflow + (flood_outflow - norm_outflow) * (self.var.ReservoirFillCC - adj_norm_fill) / (flood_fill - adj_norm_fill),
                outflow
            )
            # flood zone
            temp = np.minimum(flood_outflow, np.maximum(inflow * 1.2, norm_outflow))
            outflow = np.where(
            ...
            outflow = np.where((outflow > 1.2 * inflow) &
                               (outflow > norm_outflow) &
                               (self.var.ReservoirFillCC < flood_fill),
                               np.minimum(outflow, np.maximum(inflow, norm_outflow)),
                               outflow)
