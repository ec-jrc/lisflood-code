...
# --------------------------------------------------------------------------
    ...
    def dynamic_inloop(self,NoRoutingExecuted):
        ...
        if option['simulateReservoirs']:
            ...
            # CMmod
            # using fixed time interval (day)
            #ReservoirOutflow1 = np.minimum( self.var.MinReservoirOutflowCC, self.var.ReservoirStorageM3CC * self.var.InvDtSec) #original
            ...
            #    self.var.TotalReservoirStorageM3CC * self.var.InvDtSec, temp) #original
